...
class SWCManager():
    ...
    def build_point_tree(self):
        ...
        self.point_tree = PointTree(nodes)

    def postprocess_point_tree(self, sort=False, split=False, shift=False, extend=False):

        print("Postprocessing SWC tree...")
        # 1. Sort the tree
        if sort:
            print("  Sorting tree...")
            if self.point_tree.is_sorted:
                print("  Tree is already sorted.")
            else:
                self.point_tree.sort()

        # 2. Split the tree to sections
        if split:
            print("Splitting tree to sections...")
            if self.point_tree.is_sectioned:
                print('  Sections already exist.')
            else:
                self.point_tree.split_to_sections()

            # # 3. Merge soma in case of 3PS notation
            # print("Merging soma into a single section...")
            # if len(self.point_tree._soma_sections) == 3:
            #     self.point_tree._merge_soma()

        # 4. Shift coordinates to soma center
        if shift:
            print("Shifting coordinates to soma center...")
            self.point_tree.shift_coordinates_to_soma_center()

        # 5. Extend the tree
        if extend:
            print("Extending tree...")
            self.point_tree.extend_sections()

        self.validate_point_tree()
    ...
    def build_sec_tree(self):
        ...
        sections = self.point_tree._sections
        self.sec_tree = SectionTree(sections)
        self.validate_sec_tree()

    # VALIDATE TREES

    def validate_point_tree(self):
        check_unique_ids(self.point_tree)
        validate_connectivity(self.point_tree)

        print("SWC tree validation passed successfully")
        print(f"    is connected:{self.point_tree.is_connected:2}")
        print(f"    is sorted:   {self.point_tree.is_sorted:2}")
        print(f"    is sectioned:{self.point_tree.is_sectioned:2}")
        print(f"    is extended: {self.point_tree._is_extended:2}")
    ...
    def validate_trees_match(self):
        validate_points_match(self.point_tree, self.sec_tree)
        validate_sections_match(self.point_tree, self.sec_tree)
    ...
    def plot_3d(self, ax=None, show_points=True, show_lines=True, annotate=False, animate=True, 
        ...
        for sec in self.sec_tree.sections:
            xs = [pt.x for pt in sec.points]
            ys = [pt.y for pt in sec.points]
            zs = [pt.z for pt in sec.points]
...
def validate_points_match(point_tree, sec_tree):
    ...
    """
    sec_tree_points = [
        pt for sec in sec_tree.sections for pt in sec.points]
    if not all(sec_pt is pt for sec_pt, pt in zip(sec_tree_points, point_tree.points)):
        raise ValueError(
            "The points of the SEC tree do not match the points of the SWC tree.")
