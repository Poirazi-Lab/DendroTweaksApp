...
from reduce.reducing_methods import (reduce_subtree,
                               reduce_synapse,
...
SOMA_LABEL = "soma"
EXCLUDE_MECHANISMS = ('pas', 'na_ion', 'k_ion', 'ca_ion', 'h_ion', 'ttx_ion', 'leak')
...
def find_synapse_loc(synapse_or_segment, mapping_sections_to_subtree_index):
    ...
    if section_type == "apic":
        subtree_index = mapping_sections_to_subtree_index[("apical", section_num)]
...
def subtree_reductor(original_cell,
    ...
    soma_cable = CableParams(length=soma.L, diam=soma.diam, space_const=None,
                             cm=soma.cm, rm=1.0 / soma.gbar_leak, ra=soma.Ra, e_leak=soma.e_leak,
